      ...
      <div className={isDark ? 'divide-y divide-slate-800' : 'divide-y divide-slate-100'}>
        {Array.from({ length: rows }).map((_, i) => (
          <div key={i} className="flex gap-4 p-4">
            ...
            <div className={`h-4 rounded flex-1 ${isDark ? 'bg-slate-700' : 'bg-slate-200'}`}></div>
            <div className={`h-4 rounded w-24 ${isDark ? 'bg-slate-700' : 'bg-slate-200'}`}></div>
            <div className={`h-4 rounded w-20 ${isDark ? 'bg-slate-700' : 'bg-slate-200'}`}></div>
          </div>
        ))}
      </div>
    </div>
  ...
  return (
    <div className={`space-y-3 animate-pulse ${className}`}>
      {Array.from({ length: lines }).map((_, i) => (
        <div key={i} className={`h-4 rounded ${isDark ? 'bg-slate-700' : 'bg-slate-200'}`} style={{ width: `${100 - (i * 15)}%` }}></div>
      ))}
    </div>
  );
};
...
export const SkeletonButton = ({ className = '' }) => {
  const { theme } = useTheme();
  const isDark = theme === 'dark';
  return <div className={`h-10 rounded-lg animate-pulse w-24 ${isDark ? 'bg-slate-700' : 'bg-slate-200'} ${className}`}></div>;
};
...
export const SkeletonAvatar = ({ size = 'md', className = '' }) => {
  ...
  return (
    <div className={`${sizeClasses[size]} rounded-full animate-pulse ${isDark ? 'bg-slate-700' : 'bg-slate-200'} ${className}`}></div>
  );
};
