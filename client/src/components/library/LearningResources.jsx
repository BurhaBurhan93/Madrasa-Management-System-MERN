    ...
    <div className="w-full space-y-8 animate-in fade-in duration-500">
      ...
      <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-6">
        <div>
          ...
          <h1 className="text-4xl font-black text-slate-900 tracking-tight">{t('resourceLibrary.title')}</h1>
          <p className="text-slate-500 mt-1 font-medium italic">{t('resourceLibrary.subtitle')}</p>
        </div>
      ...
      <div className="p-10 bg-slate-900 rounded-[40px] text-white shadow-2xl shadow-slate-200/50 relative overflow-hidden group">
        <div className="relative z-10 grid grid-cols-1 lg:grid-cols-2 gap-10 items-center">
          <div>
            <Badge className="bg-white/10 text-cyan-400 border-none mb-6 px-4 py-1.5 font-black uppercase tracking-widest text-[10px]">{t('resourceLibrary.newCollection')}</Badge>
            <h2 className="text-4xl font-black tracking-tight mb-4">{t('resourceLibrary.heritageLibrary')}</h2>
            <p className="text-slate-400 text-lg font-medium leading-relaxed mb-8">
              {t('resourceLibrary.heritageDesc')}
            </p>
            <Button variant="primary" className="rounded-2xl px-10 py-4 bg-cyan-600 hover:bg-cyan-700 font-black text-xs uppercase tracking-widest shadow-xl shadow-cyan-900/20 flex items-center gap-2">
              {t('resourceLibrary.explore')} <FiArrowRight />
            </Button>
          </div>
        ...
        {resourceCards.map((res, i) => (
          <Card key={i} className="group p-8 rounded-[32px] bg-white border border-slate-100 shadow-xl shadow-slate-200/50 hover:shadow-2xl hover:border-cyan-200 transition-all duration-300 transform hover:-translate-y-1">
            ...
            <button className="flex items-center gap-2 text-xs font-black text-cyan-600 uppercase tracking-widest hover:text-cyan-700 transition-colors">
              {t('resourceLibrary.accessDatabase')} <FiExternalLink />
            </button>
          </Card>
        ))}
      </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-8">
        <Card title={t('resourceLibrary.hours')} className="rounded-[32px] p-8">
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-6">
            <div className="p-6 bg-slate-50 rounded-3xl border border-slate-100">
              <p className="text-[10px] font-black text-slate-400 uppercase tracking-widest mb-2">{t('resourceLibrary.weekdays')}</p>
              <p className="text-lg font-black text-slate-900">08:00 AM - 09:00 PM</p>
              <p className="text-xs font-bold text-emerald-600 mt-1 uppercase tracking-widest">{t('resourceLibrary.openNow')}</p>
            </div>
            <div className="p-6 bg-slate-50 rounded-3xl border border-slate-100">
              <p className="text-[10px] font-black text-slate-400 uppercase tracking-widest mb-2">{t('resourceLibrary.weekends')}</p>
              <p className="text-lg font-black text-slate-900">10:00 AM - 06:00 PM</p>
              <p className="text-xs font-bold text-slate-400 mt-1 uppercase tracking-widest">{t('resourceLibrary.selfService')}</p>
            </div>
          </div>
        </Card>
        ...
        <div className="p-8 bg-gradient-to-br from-emerald-600 to-teal-700 rounded-[32px] text-white shadow-xl shadow-emerald-200/50 relative overflow-hidden group">
          <div className="relative z-10">
            <h4 className="text-2xl font-black mb-2 tracking-tight">{t('resourceLibrary.researchSupport')}</h4>
            <p className="text-emerald-100 text-sm font-medium mb-8 leading-relaxed">
              {t('resourceLibrary.researchSupportDesc')}
            </p>
            <Button variant="outline" className="rounded-2xl px-8 py-4 border-white/20 bg-white/10 hover:bg-white/20 text-white font-black text-xs uppercase tracking-widest transition-all">
              {t('resourceLibrary.bookAppointment')}
            </Button>
          </div>
