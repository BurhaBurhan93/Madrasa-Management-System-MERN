    ...
    <div className="w-full space-y-8 animate-in fade-in duration-500">
      ...
      <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-6">
        <div>
          ...
          <h1 className="text-4xl font-black text-slate-900 tracking-tight">{t('borrowedBooks.title')}</h1>
          <p className="text-slate-500 mt-1 font-medium italic">{t('borrowedBooks.subtitle')}</p>
        </div>
        <div className="flex gap-3">
          <div className="h-12 px-6 rounded-2xl bg-white border border-slate-100 shadow-sm flex items-center gap-3">
            ...
            <span className="text-xs font-black uppercase tracking-widest text-slate-600">
              {t('borrowedBooks.limit')}: {borrowedBooks.length} / 5 {t('borrowedBooks.books')}
            </span>
          </div>
        </div>
      </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
        ...
        <div className="lg:col-span-2 space-y-6">
          <Card title={t('borrowedBooks.activeLoans')} className="rounded-[32px] p-8">
            <div className="space-y-6">
              {borrowedBooks.length > 0 ? (
                    ...
                    <div className="flex flex-col md:flex-row gap-8">
                      ...
                      <div className="flex-1 space-y-6">
                        <div className="flex justify-between items-start">
                          <div>
                            <Badge className="bg-white border-none font-black text-[10px] uppercase tracking-widest mb-2">{t('borrowedBooks.reference')} #{book.isbn?.substring(0, 8) || 'GEN-LIB'}</Badge>
                        ...
                        <div className="grid grid-cols-2 gap-6 p-4 bg-white/50 rounded-2xl border border-white">
                          <div>
                            <p className="text-[10px] font-bold text-slate-400 uppercase tracking-widest mb-1">{t('borrowedBooks.issueDate')}</p>
                          ...
                          <div>
                            <p className="text-[10px] font-bold text-slate-400 uppercase tracking-widest mb-1">{t('borrowedBooks.returnDue')}</p>
                        ...
                        <div className="flex gap-3">
                          ...
                          >
                            <FiRefreshCw /> {t('borrowedBooks.renew')}
                          </Button>
                          <Button 
                            variant="outline" 
                          ...
                          >
                            <FiCornerUpLeft /> {t('borrowedBooks.return')}
                          </Button>
                        </div>
                      </div>
                    </div>
                ...
                <div className="py-24 text-center">
                  ...
                  <p className="text-slate-400 font-bold text-sm uppercase tracking-widest">{t('borrowedBooks.noLoans')}</p>
                </div>
              )}
            </div>
          </Card>
        ...
        <div className="space-y-8">
          <Card title={t('borrowedBooks.policy')} className="rounded-[32px] p-8 bg-slate-900 text-white border-none shadow-2xl shadow-slate-900/20">
            <div className="space-y-4">
              {[
                { icon: <FiClock />, text: t('borrowedBooks.policy1') },
                { icon: <FiRefreshCw />, text: t('borrowedBooks.policy2') },
                { icon: <FiAlertCircle />, text: t('borrowedBooks.policy3') },
                { icon: <FiCheckCircle />, text: t('borrowedBooks.policy4') }
              ].map((item, i) => (
          ...
          <div className="p-8 bg-gradient-to-br from-indigo-600 to-blue-700 rounded-[40px] text-white shadow-2xl shadow-blue-200/50 relative overflow-hidden group">
            <div className="relative z-10">
              <h4 className="text-xl font-black mb-2">{t('borrowedBooks.needBook')}</h4>
              <p className="text-blue-100 text-sm font-medium mb-6 italic">{t('borrowedBooks.needBookDesc')}</p>
              <Button variant="outline" className="w-full rounded-2xl py-4 border-white/20 bg-white/10 hover:bg-white/20 text-white font-black text-xs uppercase tracking-widest transition-all">
                {t('borrowedBooks.requestAcquisition')}
              </Button>
            </div>
