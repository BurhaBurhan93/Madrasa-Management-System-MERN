...
const StudentPanelContent = () => {
  ...
  const isRTL = lang === 'dari' || lang === 'ps';
    const { t, i18n: i18nHook } = useTranslation(['student', 'common', 'nav', 'app']);
  ...
  const madrasaDisplayName = getMadrasaDisplayName(madrasaInfo);
            ...
            <div className="flex items-center gap-3">
              <div className="flex h-12 w-12 shrink-0 items-center justify-center overflow-hidden rounded-2xl bg-gradient-to-br from-cyan-500 via-sky-500 to-blue-600 text-lg font-bold text-white shadow-[0_12px_30px_-18px_rgba(14,165,233,0.9)]">
                {madrasaLogo && !logoErrored ? (
                  ...
                  <span>{madrasaDisplayName[0]?.toUpperCase() || 'M'}</span>
                )}
              </div>
              {sidebarOpen && (
                <div>
                  <div className="text-sm font-semibold uppercase tracking-[0.22em] text-cyan-600">{madrasaDisplayName}</div>
