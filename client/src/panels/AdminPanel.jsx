...
const AdminPanelContent = () => {
  ...
  const setLang = (l) => {
    setLangRaw(l);
    localStorage.setItem("lang", l === 'dari' ? 'prs' : l);
    i18n.changeLanguage(l === 'dari' ? 'prs' : l);
  };
  ...
  const [showLogoutConfirm, setShowLogoutConfirm] = useState(false);
    ...
    const { t, i18n: i18nHook } = useTranslation(['admin', 'common', 'nav', 'app']);
  ...
  const menuItems = [
    ...
    },
    {
      ...
      items: [
        ...
        { id: "reports", label: t('navReports'), path: "attendance/reports" },
        { id: "settings", label: t('navSettings'), path: "attendance/settings" },
  ...
  return (
    ...
    >
      <aside
        className={`panel-sidebar fixed z-30 h-screen border-r backdrop-blur-xl transition-all duration-300 md:relative ${theme === "dark" ? "border-slate-700/60 bg-slate-900/95" : "border-white/70 bg-white/90"} ${sidebarOpen ? "panel-sidebar-open w-[85vw] max-w-[300px] md:w-72" : "w-24"}`}
      >
