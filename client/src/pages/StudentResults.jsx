...
const StudentResults = () => {
  ...
  const fetchResultsData = async () => {
    try {
      ...
      setError(t('results.offlineError'));
  ...
  const handleExport = useCallback(() => {
    const rows = results.map(r => [
      r.exam?.title || r.exam?.subject?.name || t('academic'),
    ...
    doc.setFontSize(18);
    doc.text(t('results.pdfTitle'), 14, 20);
    doc.setFontSize(10);
    doc.text(`${t('results.pdfGpa')}: ${gpa} | ${results.length} ${t('results.pdfAssessments')}`, 14, 28);
    doc.setFontSize(8);
    doc.text(`${t('results.pdfGenerated')}: ${new Date().toLocaleDateString()}`, 14, 34);
    autoTable(doc, {
      startY: 40,
      head: [[t('results.pdfExam'), t('results.pdfSubject'), t('results.pdfScore'), t('results.pdfTotal'), t('results.pdfPercentage'), t('common:date')]],
      body: rows,
      styles: { fontSize: 9, cellPadding: 4 },
      headStyles: { fillColor: [30, 41, 59], fontSize: 9, fontStyle: 'bold' },
      alternateRowStyles: { fillColor: [248, 250, 252] },
    });
    doc.save(t('results.pdfFileName'));
  }, [results, gpa]);
    ...
    <div className="w-full space-y-8 animate-in fade-in duration-500 dark:text-gray-100">
      ...
      <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-6">
        <div>
          ...
          <h1 className="text-4xl font-black text-slate-900 dark:text-white tracking-tight">{t('myResults')}</h1>
          <p className="text-slate-500 dark:text-gray-400 mt-1 font-medium italic">{t('results.subtitle')}</p>
        </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-8">
        <div className="md:col-span-1 p-10 bg-gradient-to-br from-cyan-600 via-sky-600 to-blue-700 rounded-[40px] text-white shadow-2xl shadow-cyan-200/50 flex flex-col items-center justify-center text-center relative overflow-hidden group">
          <div className="relative z-10">
            <p className="text-[10px] font-black uppercase tracking-[0.3em] text-cyan-100 mb-4">{t('results.currentGpa')}</p>
            <h2 className="text-7xl font-black tracking-tighter mb-4">{gpa}</h2>
            <Badge className="bg-white/20 text-white border-none backdrop-blur-md px-4 py-1.5 font-black uppercase tracking-widest text-[10px]">{t('results.distinction')}</Badge>
          </div>
          {/* Decorative Pattern */}
        ...
        <div className="md:col-span-2 grid grid-cols-1 sm:grid-cols-2 gap-6">
          {[
            { label: t('results.coursesCompleted'), value: results.length, icon: <FiCheckCircle />, color: 'emerald' },
            { label: t('results.creditsEarned'), value: results.length * 3, icon: <FiTrendingUp />, color: 'blue' },
            { label: t('results.passed'), value: results.filter(r => r.totalMarks > 0 && (r.score / r.totalMarks) >= 0.5).length, icon: <FiActivity />, color: 'amber' },
            { label: t('results.failed'), value: results.filter(r => r.totalMarks > 0 && (r.score / r.totalMarks) < 0.5).length, icon: <FiAward />, color: 'rose' }
          ].map((stat, i) => (
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
        <div className="lg:col-span-2">
          <Card title={t('results.subjectPerformance')} className="rounded-[32px] p-8 dark:bg-gray-800 dark:border-gray-700">
            {performanceData.length > 0 ? (
              ...
              <div className="h-[350px] flex flex-col items-center justify-center text-slate-200">
                ...
                <p className="font-bold text-sm uppercase tracking-widest">{t('results.insufficientData')}</p>
              </div>
            )}
          </Card>
        </div>
        ...
        <div className="space-y-6">
          <Card title={t('results.detailedGrades')} className="rounded-[32px] p-8 dark:bg-gray-800 dark:border-gray-700">
            <div className="space-y-4">
              {results.length > 0 ? (
                results.map((r, i) => {
                  ...
                  <div key={i} className="p-4 rounded-2xl bg-slate-50 dark:bg-gray-700/50 border border-slate-100 dark:border-gray-700 hover:border-cyan-200 transition-colors">
                    <div className="flex justify-between items-center mb-2">
                      <p className="font-black text-slate-900 dark:text-white text-sm">{r.exam?.title || r.exam?.subject?.name || t('academic')}</p>
                ...
                <div className="text-center py-12">
                  ...
                  <p className="text-slate-400 dark:text-gray-500 font-bold text-xs uppercase tracking-widest">{t('results.noGrades')}</p>
                </div>
              )}
            </div>
          </Card>
