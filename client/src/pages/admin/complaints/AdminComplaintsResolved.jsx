...
import { FiEye, FiCheckCircle, FiStar, FiMessageSquare } from 'react-icons/fi';
...
const AdminComplaintsResolved = () => {
  ...
  const fetchData = useCallback(async () => {
    setLoading(true);
    try {
      const { data: res } = await api.get('/complaints?status=resolved');
  ...
  const columns = [
    ...
    { key: 'resolvedAt', label: t('complaints.resolvedDate'), render: (val, row) => <span className="text-slate-600">{val ? new Date(val).toLocaleDateString() : row.updatedAt ? new Date(row.updatedAt).toLocaleDateString() : '-'}</span> },
    { key: 'resolution', label: t('complaints.resolution'), render: (val) => <span className="text-slate-600 max-w-xs truncate block">{val || '-'}</span> },
    {
  ...
  return (
    <div className="w-full bg-gray-50 min-h-screen p-6">
      <div className="mb-6">
        <h1 className="text-3xl font-bold text-gray-900">{t('complaints.resolvedComplaints')}</h1>
        <p className="text-gray-600 mt-1">{t('complaints.successfullyResolved')}</p>
      </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-4 gap-4 mb-6">
        <div className="relative overflow-hidden rounded-2xl bg-gradient-to-br from-green-500 to-green-600 p-5 text-white shadow-lg">
          <div className="relative z-10"><p className="text-sm font-medium text-white/80">{t('common.resolved')}</p><p className="mt-1 text-2xl font-bold">{complaints.length}</p></div>
      ...
      <div className="rounded-2xl border border-slate-200 bg-white shadow-sm">
        <Table columns={columns} data={paginated} emptyMessage={t('complaints.noResolved')} />
      </div>
