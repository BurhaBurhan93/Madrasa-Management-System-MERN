...
const TeacherEdit = () => {
  ...
  const [form, setForm] = useState({
    ...
    gender: 'male', bloodGroup: '', maritalStatus: '', cnic: '',
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      navigate(`/admin/users/teachers/view/${id}`);
    } catch (err) {
  ...
  return (
    <div className="p-6 space-y-6">
      ...
      <div className="bg-white rounded-xl shadow-sm border border-slate-200 p-6">
        <h1 className="text-2xl font-bold text-slate-900 mb-6">{t('users.editTeacher')}</h1>
        <form onSubmit={handleSubmit} className="space-y-8">
          ...
          <div>
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
              <Field label={t('users.employeeId')} value={form.employeeCode || ''} placeholder={t('users.autoGenerated')} />
          ...
          <div className="flex gap-3 pt-4 border-t">
            ...
            <button type="button" onClick={() => navigate(`/admin/users/teachers/view/${id}`)} className="px-6 py-2 border rounded-lg hover:bg-slate-50">{t('users.cancel')}</button>
          </div>
        </form>
      </div>
    </div>
