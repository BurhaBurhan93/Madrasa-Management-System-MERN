        ...
        {loading ? renderLoading() : (
          <div className="overflow-hidden rounded-3xl border border-slate-200 bg-white shadow-sm dark:border-slate-700 dark:bg-slate-800/50">
            <table className="w-full text-left text-sm">
              ...
              <tbody>
                {filtered.map((item) => (
                  <tr key={item._id} className="border-b border-slate-100 transition last:border-0 hover:bg-slate-50 dark:border-slate-700/50 dark:hover:bg-slate-800/30">
                    <td className="px-5 py-4 font-medium text-slate-900 dark:text-slate-100">{item.className || '-'}</td>
                    <td className="px-5 py-4 text-slate-600 dark:text-slate-300">{item.subject || '-'}</td>
                    <td className="px-5 py-4 text-slate-600 dark:text-slate-300">{item.topic || '-'}</td>
                    <td className="max-w-xs truncate px-5 py-4 text-slate-600 dark:text-slate-300">{item.description || '-'}</td>
                    <td className="px-5 py-4 text-slate-600 dark:text-slate-300">{item.semester || '-'}</td>
                    <td className="px-5 py-4 text-slate-600 dark:text-slate-300">{item.order || 0}</td>
                  </tr>
                ))}
              </tbody>
            </table>
