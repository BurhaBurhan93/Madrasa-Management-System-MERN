...
const TeacherExamSubmissions = () => {
  ...
  return (
    <div className="min-h-screen w-full bg-[radial-gradient(circle_at_top,_rgba(6,182,212,0.12),_transparent_30%),linear-gradient(180deg,_#f8fafc_0%,_#eef7f7_100%)]">
      <div className="mx-auto max-w-7xl px-4 py-6 sm:px-6 lg:px-8">
        ...
        <div className="mb-8">
          <h1 className="text-2xl font-bold text-slate-900">{t('teacher.examSubmissions.title')} — {exam?.title}</h1>
          <p className="mt-1 text-sm text-slate-500"> {t('teacher.examSubmissions.totalSubmissions')}: {submissions.length} </p>
        </div>
        ...
        <div className="overflow-hidden rounded-3xl border border-slate-200 bg-white shadow-sm">
          {loading ? (
            <div className="flex h-32 items-center justify-center text-slate-500">{t('teacher.examSubmissions.loading')}</div>
          ) : (
  ...
  <tr className="border-b border-slate-100 bg-slate-50">
    {[
      '#',
      t('teacher.examSubmissions.student'),
      t('teacher.examSubmissions.score'),
      t('teacher.examSubmissions.total'),
      t('teacher.examSubmissions.percentage'),
      t('teacher.examSubmissions.status'),
      t('teacher.examSubmissions.submittedAt')
    ].map(h => (
      <th
        key={h}
        ...
        {h}
              ...
              <tbody className="divide-y divide-slate-100">
                {submissions.length === 0 ? (
                  <tr><td colSpan="7" className="p-8 text-center text-slate-500">{t('teacher.examSubmissions.noSubmissions')}</td></tr>
                ) : submissions.map((s, i) => {
                  ...
                  return (
                    <tr key={s._id} className="transition-colors duration-150 hover:bg-slate-50">
                      ...
                      <td className="p-4 font-medium text-slate-900">{s.student?.user?.name || t('teacher.examSubmissions.unknown')}</td>
