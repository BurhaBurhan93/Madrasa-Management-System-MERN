      ...
      <table className="w-full text-left text-sm">
        <thead>
          <tr className="border-b border-slate-200 dark:border-slate-700">
            <th className="px-5 py-4 font-semibold text-slate-700 dark:text-slate-300">{t('common.date')}</th>
            <th className="px-5 py-4 font-semibold text-slate-700 dark:text-slate-300">{t('common.class')}</th>
            <th className="px-5 py-4 font-semibold text-slate-700 dark:text-slate-300">{t('teacher.sessions.topic')}</th>
    ...
    <div className={PANEL_PAGE_BG}>
      <div className="px-4 py-6 sm:px-6 lg:px-8">
        <div className="mb-8 flex items-center justify-between">
          <div>
            <h1 className="text-2xl font-bold text-slate-900 dark:text-slate-100">{t('teacher.sessions.title')}</h1>
            <p className="mt-1 text-sm text-slate-500 dark:text-slate-400">{t('teacher.sessions.subtitle')}</p>
          </div>
          <button
        ...
        <div className="mb-6 rounded-3xl border border-slate-200 bg-white p-5 shadow-sm dark:border-slate-700 dark:bg-slate-800/50">
          <p className="text-sm text-slate-500 dark:text-slate-400">
            {t('teacher.sessions.totalSessions')}: <span className="font-semibold text-slate-700 dark:text-slate-300">{sessions.length}</span>
          </p>
        </div>
        ...
        {loading ? renderLoading() : sessions.length === 0 ? (
          <div className="flex flex-col items-center justify-center py-20">
            ...
            <p className="text-sm font-medium text-slate-500 dark:text-slate-400">{t('teacher.sessions.noSessions')}</p>
            <button
              onClick={() => navigate('/teacher/attendance/create-session')}
