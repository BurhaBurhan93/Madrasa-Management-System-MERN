...
const TeacherSessions = () => {
 ...
 const renderTable = () => (
  ...
  <th className="px-3 sm:px-5 py-3 sm:py-4 font-semibold text-slate-700 dark:text-slate-300 whitespace-nowrap">{t('common.date')}</th>
  <th className="px-3 sm:px-5 py-3 sm:py-4 font-semibold text-slate-700 dark:text-slate-300 whitespace-nowrap">{t('common.class')}</th>
  <th className="hidden sm:table-cell px-3 sm:px-5 py-3 sm:py-4 font-semibold text-slate-700 dark:text-slate-300 whitespace-nowrap">{t('sessions.topic')}</th>
  ...
  <tbody>
  {sessions.map((s) => (
  <tr key={s._id} className="border-b border-slate-100 transition last:border-0 dark:border-slate-700/50 dark:">
  <td className="px-3 sm:px-5 py-3 sm:py-4 font-medium text-slate-900 dark:text-slate-100 whitespace-nowrap">{formatDate(s.sessionDate)}</td>
  <td className="px-3 sm:px-5 py-3 sm:py-4 text-slate-600 dark:text-slate-300 whitespace-nowrap">
  {s.class?.name ? (s.class.section ? `${s.class.name} - ${s.class.section}` : s.class.name) : '-'}
  </td>
  <td className="hidden sm:table-cell px-3 sm:px-5 py-3 sm:py-4 text-slate-600 dark:text-slate-300 truncate max-w-[200px]">{s.topic || '-'}</td>
  <td className="px-3 sm:px-5 py-3 sm:py-4">
  <button
  onClick={() => navigate('/teacher/attendance')}
  className="rounded-xl border border-cyan-200 bg-cyan-50 px-2 sm:px-3 py-1 sm:py-1.5 text-xs font-medium text-cyan-700 transition hover:bg-cyan-100 dark:border-cyan-800 dark:bg-cyan-900/30 dark:text-cyan-400 dark:hover:bg-cyan-900/50"
  >
 ...
 <div className={PANEL_PAGE_BG}>
  ...
  <div>
  <h1 className="text-xl sm:text-2xl font-bold text-slate-900 dark:text-slate-100">{t('sessions.title')}</h1>
  <p className="mt-1 text-xs sm:text-sm text-slate-500 dark:text-slate-400">{t('sessions.subtitle')}</p>
  </div>
  <button
  ...
  {t('sessions.totalSessions')}: <span className="font-semibold text-slate-700 dark:text-slate-300">{sessions.length}</span>
  </p>
  </div>
  ...
  <p className="text-xs sm:text-sm font-medium text-slate-500 dark:text-slate-400">{t('sessions.noSessions')}</p>
  <button
  onClick={() => navigate('/teacher/attendance/create-session')}
  className="mt-4 rounded-2xl bg-cyan-600 px-4 sm:px-5 py-2 sm:py-2.5 text-xs sm:text-sm font-medium text-white shadow-sm transition hover:-translate-y-0.5 hover:bg-cyan-700"
  >
