  ...
  return (
    <div className="min-h-screen w-full bg-[radial-gradient(circle_at_top,_rgba(6,182,212,0.12),_transparent_30%),linear-gradient(180deg,_#f8fafc_0%,_#eef7f7_100%)]">
      <div className="mx-auto max-w-7xl px-4 py-6 sm:px-6 lg:px-8">
        ...
        <div className="mb-8">
          <h1 className="text-2xl font-bold text-slate-900">{t('teacher.viewResults.title')}</h1>
          <p className="mt-1 text-sm text-slate-500">{t('teacher.viewResults.subtitle')}</p>
        </div>
        ...
        <div className="mb-6 rounded-3xl border border-slate-200 bg-white p-5 shadow-sm">
          <div className="grid grid-cols-1 gap-4 md:grid-cols-4 items-end">
            <div>
              <label className="mb-1 block text-xs font-medium text-slate-500">{t('teacher.viewResults.exam')}</label>
            ...
            <div>
              <label className="mb-1 block text-xs font-medium text-slate-500">{t('teacher.viewResults.allSubjects')}</label>
            ...
            <div>
              <label className="mb-1 block text-xs font-medium text-slate-500">{t('teacher.viewResults.class')}</label>
        ...
        <section className="grid gap-4 md:grid-cols-2 xl:grid-cols-4 mb-6">
          {[
            { label: t('teacher.viewResults.total'), value: stats.total, accent: 'bg-cyan-500' },
            ...
            { label: t('teacher.viewResults.average'), value: stats.avg, accent: 'bg-sky-500' },
          ].map(c => (
        ...
        <div className="overflow-hidden rounded-3xl border border-slate-200 bg-white shadow-sm">
          {loading ? (
            <div className="flex h-32 items-center justify-center text-slate-500">{t('teacher.viewResults.loading')}</div>
          ) : (
            <table className="w-full text-sm">
              <thead>
                <tr className="border-b border-slate-100 bg-slate-50">
                  {[
  '#',
  t('teacher.viewResults.student'),
  t('teacher.viewResults.exam'),
  t('teacher.viewResults.subject'),
  t('teacher.viewResults.score'),
  t('teacher.viewResults.grade'),
  t('teacher.viewResults.status')
].map(h => (
              ...
              <tbody className="divide-y divide-slate-100">
                {results.length === 0 ? (
                  <tr><td colSpan="7" className="p-8 text-center text-slate-500">{t('teacher.viewResults.noResults')}</td></tr>
                ) : results.map((r, i) => (
                  <tr key={r._id} className="transition-colors duration-150 hover:bg-slate-50">
                    ...
                    <td className="p-4">
                      <span className={`rounded-full px-3 py-1 text-xs font-medium ${r.status === 'pass' ? 'bg-emerald-100 text-emerald-700' : 'bg-rose-100 text-rose-700'}`}>
                        {r.status === 'pass'
    ? t('teacher.viewResults.pass')
    : t('teacher.viewResults.fail')}
                      </span>
                    </td>
                  </tr>
                ))}
