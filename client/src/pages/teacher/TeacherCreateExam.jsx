...
const TeacherCreateExam = () => {
 ...
 <div className={PANEL_PAGE_BG}>
  ...
  {editId ? t('createExam.editExam') : t('createExam.createExam')}
  </h1>
  <p className="mt-1 text-xs sm:text-sm text-slate-500 dark:text-slate-400">
  {editId ? t('createExam.editSubtitle') : t('createExam.createSubtitle')}
  </p>
  </div>
  ...
  {t('createExam.assignmentTitle')} <span className="text-rose-500">*</span>
  </label>
  <input
  type="text"
  value={form.title}
  ...
  required
  className={fieldCls}
  placeholder={t('createExam.assignmentTitlePlaceholder')}
  />
  </div>
 ...
 className={fieldCls}
 >
 <option value="">{t('createExam.selectSubject')}</option>
 {subjects.map((s) => (
 ...
 className={fieldCls}
 >
 <option value="">{t('createExam.selectClass')}</option>
 {classes.map((c) => (
 <option key={c._id} value={c._id}>{c.name} {c.section}</option>
  ...
  <div>
  <label className="mb-1 block text-xs font-medium text-slate-500 dark:text-slate-400">
  {t('createExam.duration')} <span className="text-rose-500">*</span>
 </label>
 <input
 type="number"
 value={form.duration}
 ...
 required
 min="1" className={fieldCls}
 />
 </div>

 <div>
 <label className="mb-1 block text-xs font-medium text-slate-500 dark:text-slate-400">
 {t('createExam.academicYear')}
 </label>
 <input
 type="text"
 ...
 />
  ...
  <div>
  <label className="mb-1 block text-xs font-medium text-slate-500 dark:text-slate-400">
  {t('createExam.startDate')}
 </label>
 <CalendarDatePicker
 value={form.startDate}
 onChange={(date) => setForm({ ...form, startDate: date })}
 placeholder={t('createExam.selectStartDate')}
 />
 </div>

 <div>
 <label className="mb-1 block text-xs font-medium text-slate-500 dark:text-slate-400">
 {t('createExam.endDate')}
 </label>
 <CalendarDatePicker
 value={form.endDate}
 onChange={(date) => setForm({ ...form, endDate: date })}
 placeholder={t('createExam.selectEndDate')}
 />
 </div>
 </div>
  ...
  type="button"
  onClick={() => navigate(-1)}
  className="w-full sm:w-auto rounded-2xl border border-slate-200 px-4 py-2 text-sm font-medium text-slate-600 transition-all duration-200 dark:border-slate-600 dark:text-slate-400 dark:"
  >
  {t('common.cancel')}
  </button>
  ...
  type="submit"
  disabled={loading}
  className="w-full sm:w-auto rounded-2xl bg-cyan-600 px-5 py-2 text-sm font-medium text-white shadow-sm transition-all duration-200 hover:bg-cyan-700 disabled:opacity-60"
  >
  {loading ? t('common.saving') : (editId ? t('common.update') : t('createExam.createAndAddQuestions'))}
  </button>
  </div>
 </form>
 </div>
