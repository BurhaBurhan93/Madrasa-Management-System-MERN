...
const TeacherFeedback = () => {
 ...
 if (!form.subject.trim()) { alert(t('feedback.subjectRequired')); return; }
 try {
 ...
 else alert(data.message || t('feedback.createFailed'));
 } catch { alert(t('feedback.createFailed')); }
 };
 ...
 else alert(data.message || t('feedback.updateFailed'));
 } catch { alert(t('feedback.updateFailed')); }
 };
 ...
 else alert(data.message || t('feedback.deleteFailed'));
 } catch { alert(t('feedback.deleteFailed')); }
 };
 ...
 <div className={PANEL_PAGE_BG}>
  ...
  <div>
  <h1 className="text-xl sm:text-2xl font-bold text-slate-900 dark:text-slate-100">{t('feedback.title')}</h1>
  <p className="mt-1 text-xs sm:text-sm text-slate-500 dark:text-slate-400">{t('feedback.subtitle')}</p>
  </div>
  <button onClick={openCreate} className="flex items-center gap-2 rounded-2xl bg-cyan-600 px-4 sm:px-5 py-2 sm:py-2.5 text-xs sm:text-sm font-medium text-white shadow-sm transition hover:bg-cyan-700">
  <FiPlus size={14} /> {t('feedback.newFeedback')}
  </button>
 </div>
 ...
 {/* Stats */}
  ...
  {[
  { label: t('feedback.totalFeedback'), value: summary.total, accent: 'bg-cyan-500' },
  { label: t('feedback.avgRating'), value: summary.avgRating + '/5', accent: 'bg-amber-500' },
  { label: t('feedback.studentFeedback'), value: summary.student, accent: 'bg-violet-500' },
  { label: t('feedback.selfReflections'), value: summary.self, accent: 'bg-emerald-500' },
  ].map(c => (
 ...
 {/* Filters */}
  ...
  <input type="text" placeholder={t('feedback.searchPlaceholder')} value={search} onChange={e => setSearch(e.target.value)} className="min-w-[140px] sm:min-w-0 flex-1 rounded-2xl border border-slate-200 px-3 sm:px-4 py-2 text-xs sm:text-sm text-slate-600 outline-none focus:border-cyan-300 dark:border-slate-600 dark:text-slate-200" />
  <select value={filterType} onChange={e => setFilterType(e.target.value)} className="rounded-2xl border border-slate-200 bg-white px-3 py-2 text-xs sm:text-sm text-slate-600 outline-none dark:border-slate-600 dark:text-slate-200">
 <option value="">{t('feedback.allTypes')}</option>
 {types.map(type => <option key={type} value={type}>{t('feedback.type.' + type)}</option>)}
 </select>
 <select value={filterCategory} onChange={e => setFilterCategory(e.target.value)} className="rounded-2xl border border-slate-200 bg-white px-3 py-2 text-sm text-slate-600 outline-none dark:border-slate-600 dark:text-slate-200">
 <option value="">{t('feedback.allCategories')}</option>
 {categories.map(c => <option key={c} value={c}>{t('feedback.category.' + c)}</option>)}
 </select>
  <button onClick={fetchFeedbacks} className="rounded-2xl bg-cyan-600 px-4 py-2 text-xs sm:text-sm font-medium text-white hover:bg-cyan-700">{t('common.apply')}</button>
 </div>
 </div>
 ...
 <div className="flex flex-col items-center justify-center py-20 rounded-3xl border border-slate-200 bg-white dark:border-slate-700 ">
 <p className="text-sm font-medium text-slate-500 dark:text-slate-400">{t('feedback.noFeedback')}</p>
 </div>
 ) : filtered.map(f => (
  <div key={f._id} className="rounded-2xl sm:rounded-3xl border border-slate-200 bg-white p-4 sm:p-5 shadow-sm transition hover:-translate-y-0.5 hover:shadow-md dark:border-slate-700 ">
  <div className="flex items-start justify-between gap-3 sm:gap-4">
 <div className="flex-1">
 <div className="flex items-center gap-2 flex-wrap mb-2">
 <span className={`rounded-full px-2.5 py-0.5 text-xs font-semibold ${typeColors[f.feedbackType]}`}>{t('feedback.type.' + f.feedbackType)}</span>
 <span className={`rounded-full px-2.5 py-0.5 text-xs font-semibold ${categoryColors[f.category]}`}>{t('feedback.category.' + f.category)}</span>
 ...
 <FiStar key={n} size={14} className={n <= (f.rating || 0) ? 'fill-amber-400 text-amber-400' : 'text-slate-300 dark:text-slate-600'} />
  ...
  <h3 className="text-sm sm:text-base font-semibold text-slate-900 dark:text-slate-100">{f.subject}</h3>
 {f.student && (
 <p className="mt-1 text-xs text-cyan-600 dark:text-cyan-400">
 {t('feedback.student')}: {f.student.user?.name || t('common.unknown')} ({f.student.studentCode || t('common.na')})
 </p>
 )}
  <p className="mt-2 text-xs sm:text-sm text-slate-600 dark:text-slate-300 whitespace-pre-wrap">{f.comments}</p>
 <div className="mt-3 flex flex-wrap items-center gap-3 text-xs text-slate-400 dark:text-slate-500">
 <span>{formatDate(f.createdAt)}</span>
 {f.class && <span>{t('feedback.feedbackClass')}: {f.class.name} {f.class.section || ''}</span>}
 {f.subjectRef && <span>{t('feedback.feedbackSubject')}: {f.subjectRef.name}</span>}
 {f.tags?.length > 0 && (
 <div className="flex gap-1">
 {f.tags.map(tag => (
 <span key={tag} className="rounded-full px-2 py-0.5 text-[10px] font-medium text-slate-500 dark:text-slate-400">#{tag}</span>
 ))}
 </div>
 )}
 </div>
 ...
 <div className="w-full max-w-lg rounded-3xl border border-slate-200 bg-white p-6 shadow-2xl dark:border-slate-700 max-h-[90vh] overflow-y-auto">
 <div className="mb-5 flex items-start justify-between">
 <h2 className="text-lg font-semibold text-slate-900 dark:text-slate-100">{editing ? t('feedback.editFeedback') : t('feedback.newFeedback')}</h2>
 <button onClick={() => { setShowForm(false); setEditing(null); }} className="flex h-8 w-8 items-center justify-center rounded-xl border border-slate-200 text-slate-500 dark:border-slate-600 dark:text-slate-400 dark:">×</button>
 </div>
 <div className="space-y-4">
 <div className="grid grid-cols-2 gap-4">
 <div>
 <label className={labelCls}>{t('feedback.feedbackType')}</label>
 <select value={form.feedbackType} onChange={e => setForm({ ...form, feedbackType: e.target.value })} className={fieldCls}>
 {types.map(type => <option key={type} value={type}>{t('feedback.type.' + type)}</option>)}
 </select>
 </div>
 <div>
 <label className={labelCls}>{t('feedback.feedbackCategory')}</label>
 ...
 <div>
 <label className={labelCls}>{t('feedback.subject')}</label>
 <input value={form.subject} onChange={e => setForm({ ...form, subject: e.target.value })} className={fieldCls} placeholder={t('feedback.subjectPlaceholder')} />
 </div>
 <div>
 <label className={labelCls}>{t('feedback.rating')}</label>
 <div className="flex gap-1">
 {[1,2,3,4,5].map(n => (
 ...
 <div>
 <label className={labelCls}>{t('feedback.comments')}</label>
 <textarea value={form.comments} onChange={e => setForm({ ...form, comments: e.target.value })} className={fieldCls} rows={4} placeholder={t('feedback.commentsPlaceholder')} />
 </div>
 <div>
 <label className={labelCls}>{t('feedback.tags')}</label>
 <input value={form.tags} onChange={e => setForm({ ...form, tags: e.target.value })} className={fieldCls} placeholder={t('feedback.tagsPlaceholder')} />
 </div>
 </div>
 <div className="mt-5 flex justify-end gap-3">
 <button onClick={() => { setShowForm(false); setEditing(null); }} className="rounded-2xl border border-slate-200 px-4 py-2 text-sm font-medium text-slate-600 dark:border-slate-600 dark:text-slate-200 dark:">{t('common.cancel')}</button>
