...
const TeacherFeedback = () => {
  ...
  const handleEdit = async () => {
    ...
    } catch { alert('Failed to update feedback'); }
  };
    ...
    <div className={PANEL_PAGE_BG}>
      <div className="px-4 py-6 sm:px-6 lg:px-8">
        <div className="mb-6 flex items-start justify-between">
          <div>
            <h1 className="text-2xl font-bold text-slate-900 dark:text-slate-100">My Feedback</h1>
            <p className="mt-1 text-sm text-slate-500 dark:text-slate-400">Manage your feedback for students and self-reflections</p>
          </div>
        ...
        <section className="mb-6 grid grid-cols-2 gap-4 md:grid-cols-4">
          {[
            { label: 'Total Feedback', value: summary.total, accent: 'bg-cyan-500' },
            { label: 'Avg Rating', value: summary.avgRating + '/5', accent: 'bg-amber-500' },
            { label: 'Student Feedback', value: summary.student, accent: 'bg-violet-500' },
            { label: 'Self Reflections', value: summary.self, accent: 'bg-emerald-500' },
          ].map(c => (
        ...
        <div className="mb-6 rounded-3xl border border-slate-200 bg-white p-5 shadow-sm dark:border-slate-700 dark:bg-slate-800/50">
          <div className="flex flex-wrap gap-3 items-center">
            ...
            <input type="text" placeholder="Search feedback..." value={search} onChange={e => setSearch(e.target.value)} className="flex-1 rounded-2xl border border-slate-200 px-4 py-2 text-sm text-slate-600 outline-none focus:border-cyan-300 dark:border-slate-600 dark:bg-slate-800 dark:text-slate-200 min-w-[200px]" />
            ...
            <button onClick={fetchFeedbacks} className="rounded-2xl bg-cyan-600 px-4 py-2 text-sm font-medium text-white hover:bg-cyan-700">Apply</button>
          </div>
        </div>
        ...
        <div className="space-y-4">
          {loading ? (
            ...
            <div className="flex flex-col items-center justify-center py-20 rounded-3xl border border-slate-200 bg-white dark:border-slate-700 dark:bg-slate-800/50">
              <p className="text-sm font-medium text-slate-500 dark:text-slate-400">No feedback found</p>
            </div>
          ) : filtered.map(f => (
              ...
              <div className="flex items-start justify-between gap-4">
                <div className="flex-1">
                  <div className="flex items-center gap-2 flex-wrap mb-2">
                    <span className={`rounded-full px-2.5 py-0.5 text-xs font-semibold ${typeColors[f.feedbackType]}`}>{f.feedbackType}</span>
                    <span className={`rounded-full px-2.5 py-0.5 text-xs font-semibold ${categoryColors[f.category]}`}>{f.category}</span>
                  ...
                  {f.student && (
                    <p className="mt-1 text-xs text-cyan-600 dark:text-cyan-400">
                      Student: {f.student.user?.name || 'Unknown'} ({f.student.studentCode || '-'})
                    </p>
                  )}
                  ...
                  <div className="mt-3 flex flex-wrap items-center gap-3 text-xs text-slate-400 dark:text-slate-500">
                    ...
                    {f.class && <span>Class: {f.class.name} {f.class.section || ''}</span>}
                    {f.subjectRef && <span>Subject: {f.subjectRef.name}</span>}
                    {f.tags?.length > 0 && (
                ...
                <div className="flex gap-1.5 shrink-0">
                  <button onClick={() => openEdit(f)} title="Edit" className="flex h-8 w-8 items-center justify-center rounded-xl border border-amber-200 bg-amber-50 text-amber-600 transition hover:bg-amber-100 dark:border-amber-800 dark:bg-amber-900/30 dark:text-amber-400"><FiEdit2 size={14} /></button>
                  <button onClick={() => handleDelete(f._id)} title="Delete" className="flex h-8 w-8 items-center justify-center rounded-xl border border-rose-200 bg-rose-50 text-rose-600 transition hover:bg-rose-100 dark:border-rose-800 dark:bg-rose-900/30 dark:text-rose-400"><FiTrash2 size={14} /></button>
                </div>
              </div>
            </div>
          ))}
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-slate-950/40 backdrop-blur-sm">
          <div className="w-full max-w-lg rounded-3xl border border-slate-200 bg-white p-6 shadow-2xl dark:border-slate-700 dark:bg-slate-800 max-h-[90vh] overflow-y-auto">
            <div className="mb-5 flex items-start justify-between">
              <h2 className="text-lg font-semibold text-slate-900 dark:text-slate-100">{editing ? 'Edit Feedback' : 'New Feedback'}</h2>
            ...
            <div className="space-y-4">
              ...
              <div>
                ...
                <input value={form.subject} onChange={e => setForm({ ...form, subject: e.target.value })} className={fieldCls} placeholder="e.g. Ahmad Khan - Excellent Quran recitation" />
              </div>
              <div>
              ...
              <div>
                ...
                <textarea value={form.comments} onChange={e => setForm({ ...form, comments: e.target.value })} className={fieldCls} rows={4} placeholder="Detailed feedback..." />
              </div>
              <div>
                <label className={labelCls}>Tags (comma separated)</label>
                <input value={form.tags} onChange={e => setForm({ ...form, tags: e.target.value })} className={fieldCls} placeholder="excellent, attention-needed, self-reflection" />
              </div>
            </div>
            <div className="mt-5 flex justify-end gap-3">
              <button onClick={() => { setShowForm(false); setEditing(null); }} className="rounded-2xl border border-slate-200 px-4 py-2 text-sm font-medium text-slate-600 hover:bg-slate-100 dark:border-slate-600 dark:text-slate-200 dark:hover:bg-slate-700">Cancel</button>
              <button onClick={editing ? handleEdit : handleCreate} className="rounded-2xl bg-cyan-600 px-5 py-2 text-sm font-medium text-white shadow-sm hover:bg-cyan-700">{editing ? 'Update' : 'Create'}</button>
            </div>
          </div>
        </div>
      )}
