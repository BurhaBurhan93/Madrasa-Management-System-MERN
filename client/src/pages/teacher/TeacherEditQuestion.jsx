...
const TeacherEditQuestion = () => {
 ...
 if (form.questionType === 'truefalse') payload.options = ['True', 'False'];
  await api.put(`/teacher/exams/${examId}/questions/${questionId}`, payload);
 ...
 <button type="button" onClick={() => navigate(`/teacher/exams/${examId}`)} className="rounded-2xl border border-slate-200 px-4 py-2 text-sm font-medium text-slate-600 transition-all duration-200 ">{t('teacher.editQuestion.cancel')}</button>
