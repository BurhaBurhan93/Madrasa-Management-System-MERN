...
const StudentTimetable = () => {
  ...
  const [studentInfo, setStudentInfo] = useState({
    name: '',
    class: '',
    semester: t('timetable.currentSemester')
  });
  ...
  const fetchTimetable = async () => {
    try {
      ...
      if (profile) {
        setStudentInfo({
          name: profile.name || '',
          class: profile.currentClass?.name || t('timetable.defaultLevel'),
          semester: profile.currentSemester || t('timetable.defaultAcademicYear')
        });
      }
      ...
      const scheduleData = courses.map((course, index) => ({
        ...
        teacher: course.teacher?.name || course.teacherName || t('instructor'),
        day: getDayForCourse(course, index),
        time: course.startTime && course.endTime
          ? `${course.startTime} - ${course.endTime}`
          : t('timetable.defaultTime'),
        room: t('common:na'),
        credits: course.credits || 3,
        type: course.type || t('timetable.defaultType')
      }));

      setTimetable(scheduleData.length > 0 ? scheduleData : MOCK_TIMETABLE);
    } catch (err) {
      console.error('[StudentTimetable] Error:', err);
      setError(t('timetable.offlineError'));
      setTimetable(MOCK_TIMETABLE);
    } finally {
      setLoading(false);
    }
  ...
  const handleExport = useCallback(() => {
    ...
    days.forEach(day => {
      (groupedTimetable[day] || []).forEach(slot => {
        rows.push([t('common:' + day.toLowerCase()), slot.subject, slot.teacher, slot.time, slot.room, `${slot.credits} ${t('timetable.pdfCr')}`, slot.type]);
      });
    });
    const doc = new jsPDF({ orientation: 'landscape' });
    doc.setFontSize(18);
    doc.text(t('timetable.pdfTitle'), 14, 20);
    doc.setFontSize(10);
    doc.text(`${t('timetable.pdfStudent')}: ${studentInfo.name} | ${studentInfo.semester}`, 14, 28);
    doc.setFontSize(8);
    doc.text(`${t('timetable.pdfGenerated')}: ${new Date().toLocaleDateString()}`, 14, 34);
    autoTable(doc, {
      startY: 40,
      head: [[t('common:day'), t('common:subject'), t('instructor'), t('common:time'), t('timetable.pdfRoom'), t('timetable.pdfCredits'), t('timetable.pdfType')]],
      body: rows,
      styles: { fontSize: 9, cellPadding: 4 },
      headStyles: { fillColor: [30, 41, 59], fontSize: 9, fontStyle: 'bold' },
      alternateRowStyles: { fillColor: [248, 250, 252] },
    });
    doc.save(t('timetable.pdfFileName'));
    ...
    <div className="w-full space-y-8 animate-in fade-in duration-500 dark:text-gray-100">
      ...
      <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-6">
        <div>
          ...
          <h1 className="text-4xl font-black text-slate-900 dark:text-white tracking-tight">{t('myTimetable')}</h1>
          <p className="text-slate-500 dark:text-gray-400 mt-1 font-medium italic">{t('timetable.subtitle', { semester: studentInfo.semester })}</p>
        </div>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6">
        {[
          { label: t('timetable.totalClasses'), value: timetable.length, icon: <FiBookOpen />, color: 'blue' },
          { label: t('timetable.currentClass'), value: studentInfo.class, icon: <FiTrendingUp />, color: 'emerald' },
          { label: t('academicYear'), value: t('timetable.academicYearValue'), icon: <FiCalendar />, color: 'purple' },
          { label: t('timetable.activeDays'), value: t('timetable.activeDaysValue'), icon: <FiActivity />, color: 'cyan' }
        ].map((stat, i) => (
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
        <div className="lg:col-span-2 space-y-6">
          <Card title={t('timetable.masterSchedule')} className="rounded-[32px] p-8 dark:bg-gray-800 dark:border-gray-700">
                  ...
                  <div className="space-y-4">
                    {groupedTimetable[day]?.length > 0 ? (
                      groupedTimetable[day].map((slot, idx) => (
                        <div key={idx} className="group flex flex-col md:flex-row md:items-center justify-between p-6 rounded-3xl bg-slate-50 dark:bg-gray-700/50 border border-slate-100 dark:border-gray-700 hover:border-cyan-200 hover:bg-white dark:hover:bg-gray-700 transition-all duration-300">
                          <div className="flex items-center gap-6 mb-4 md:mb-0">
                            <div className="w-14 h-14 rounded-2xl bg-transparent shadow-sm flex flex-col items-center justify-center text-cyan-600 group-hover:scale-110 transition-transform">
                              <FiClock className="text-xl" />
                            </div>
                            <div>
                              <div className="flex items-center gap-2 mb-1">
                                <p className="text-[10px] font-black text-cyan-600 uppercase tracking-[0.2em]">{t('timetable.' + slot.type?.toLowerCase(), slot.type)}</p>
                                <span className="text-slate-300">•</span>
                                <p className="text-[10px] font-black text-slate-400 uppercase tracking-widest">{t('timetable.slotCredits', { credits: slot.credits })}</p>
                              </div>
                      ...
                      <div className="py-8 text-center bg-slate-50/50 dark:bg-gray-800/50 rounded-3xl border border-dashed border-slate-200 dark:border-gray-700">
                          <p className="text-xs font-bold text-slate-400 dark:text-gray-500 uppercase tracking-widest italic">{t('timetable.noClasses')}</p>
                      </div>
                    )}
                  </div>
                </div>
        ...
        <div className="space-y-8">
          <Card title={t('timetable.attendancePolicy')} className="rounded-[32px] p-8 bg-slate-900 text-white border-none shadow-2xl shadow-slate-900/20">
            <div className="space-y-4">
              {[
                { icon: <FiClock />, text: t('timetable.policy1') },
                { icon: <FiInfo />, text: t('timetable.policy2') },
                { icon: <FiActivity />, text: t('timetable.policy3') },
                { icon: <FiCheckCircle />, text: t('timetable.policy4') }
              ].map((item, i) => (
          ...
          <div className="p-8 bg-gradient-to-br from-cyan-600 to-blue-700 rounded-[32px] text-white shadow-2xl shadow-cyan-200/50 relative overflow-hidden group">
            <div className="relative z-10">
              <h4 className="text-xl font-black mb-2">{t('timetable.academicCalendar')}</h4>
              <p className="text-cyan-100 text-sm font-medium mb-6">{t('timetable.calendarDesc')}</p>
