...
const StudentEvents = () => {
  ...
  const [newEvent, setNewEvent] = useState({ title: '', date: '', time: '', location: '', description: '', type: t('events.typePersonal') });
  ...
  const fetchEventsData = async () => {
    try {
      ...
      const examEvents = unwrapArrayResponse(examsResponse.data).map(exam => ({
        ...
        title: exam.title || t('events.defaultExamTitle'),
        date: exam.date || exam.publishDate || new Date().toISOString(),
        time: exam.startTime || t('events.defaultTime'),
        location: exam.location || t('events.examinationHall'),
        type: t('events.typeExamination'),
        description: exam.description || t('events.defaultExamDesc', { title: exam.title }),
      ...
      const assignmentEvents = unwrapArrayResponse(assignmentsResponse.data)
        ...
        .map(assignment => ({
          ...
          title: assignment.title || t('events.defaultAssignmentTitle'),
          date: assignment.dueDate,
          time: t('events.defaultDueTime'),
          location: t('events.submissionPortal'),
          type: t('events.typeAssignment'),
          description: assignment.description || t('events.defaultAssignmentDesc', { title: assignment.title }),
      ...
      const customEvents = unwrapArrayResponse(eventsResponse.data).map(event => ({
        ...
        title: event.title,
        date: event.date,
        time: event.time || t('events.defaultNoonTime'),
        location: event.location || t('common:na'),
        type: event.type || t('events.typePersonal'),
  ...
  const handleAddEvent = async (e) => {
    ...
    if (!newEvent.title || !newEvent.date) {
      setAddError(t('events.titleDateRequired'));
      return;
    }
    try {
      setLoading(true);
      ...
      const custom = {
        ...
        date: newEvent.date,
        time: newEvent.time || t('events.defaultNoonTime'),
        location: newEvent.location || t('common:na'),
        type: newEvent.type || t('events.typePersonal'),
    ...
    setNewEvent({ title: '', date: '', time: '', location: '', description: '', type: t('events.typePersonal') });
    ...
    <div className="w-full space-y-8 animate-in fade-in duration-500">
      ...
      <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-6">
        <div>
          ...
          <h1 className="text-4xl font-black text-slate-900 tracking-tight">{t('events.label', { ns: 'student' })}</h1>
          <p className="text-slate-500 mt-1 font-medium italic">{t('events.subtitle')}</p>
        </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
        ...
        <div className="lg:col-span-2 space-y-6">
          <Card title={t('events.institutionalTimeline')} className="rounded-[32px] p-8">
              ...
              {filteredEvents.length > 0 ? (
                filteredEvents.map((event) => (
                  <div key={event.id} className="group flex gap-8">
                    ...
                    {/* Content Card */}
                    <div className="flex-1 p-6 rounded-3xl bg-transparent border border-slate-100 group-hover:border-cyan-200 group-hover:bg-white transition-all duration-300">
                ...
                <div className="py-20 text-center">
                  ...
                  <p className="text-slate-400 font-bold text-sm uppercase tracking-widest">{t('events.noEventsScheduled')}</p>
                </div>
              )}
            </div>
          </Card>
        ...
        <div className="space-y-8">
          ...
          <Card title={t('events.eventStatistics')} className="rounded-[32px] p-8">
            <PieChartComponent 
              data={[
                ...
                { name: t('events.examinations'), value: events.filter(e => e.type === 'Examination').length, color: '#F43F5E' },
                { name: t('events.assignments'), value: events.filter(e => e.type === 'Assignment').length, color: '#F59E0B' }
              ].filter(item => item.value > 0)}
              height={250}
            />
          </Card>

          <Card title={t('events.monthlyActivity')} className="rounded-[32px] p-8">
            <BarChartComponent 
              data={[
                { name: t('events.thisMonth'), value: events.filter(e => {
                  const eventDate = new Date(e.date);
                  const now = new Date();
                  return eventDate.getMonth() === now.getMonth();
                }).length },
                { name: t('events.nextMonth'), value: events.filter(e => {
                  const eventDate = new Date(e.date);
          ...
          <div className="p-8 bg-gradient-to-br from-slate-900 via-slate-800 to-slate-900 rounded-[40px] text-white shadow-2xl shadow-slate-200/50 text-center relative overflow-hidden group">
            <div className="relative z-10">
              <p className="text-[10px] font-black text-cyan-400 uppercase tracking-[0.3em] mb-4">{t('events.todaysDate')}</p>
              <h2 className="text-7xl font-black tracking-tighter mb-2">{new Date().getDate()}</h2>
              <p className="text-xl font-bold text-slate-300">
                {new Date().toLocaleString('default', { month: 'long', year: 'numeric' })}
              </p>
              <div className="mt-8 flex justify-center">
                <Badge variant="primary" className="px-4 py-2">{t('events.systemActive')}</Badge>
              </div>
            </div>
          ...
          <Card title={t('events.quickStats')} className="rounded-[32px] p-8">
            <div className="space-y-6">
              {[
                ...
                { label: t('events.examinations'), value: events.filter(e => e.type === 'Examination').length, color: 'rose' }
              ].map((stat, i) => (
        ...
        <h4 className="text-xl font-black mb-2">{t('events.needAHall')}</h4>
            <p className="text-blue-100 text-sm font-medium mb-6">{t('events.requestRoomBooking')}</p>
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/40 backdrop-blur-sm p-4">
          <div className="bg-white rounded-[32px] shadow-2xl w-full max-w-md p-8">
            <h2 className="text-xl font-black text-slate-900 mb-6 flex items-center gap-2">
              <FiPlus className="text-cyan-600" /> {t('events.addPersonalEvent')}
            </h2>
            <form onSubmit={handleAddEvent} className="space-y-4">
              {addError && <p className="text-red-500 text-sm">{addError}</p>}
              <div>
                <label className="block text-xs font-bold text-slate-500 uppercase tracking-widest mb-1">{t('events.titleLabel')}</label>
                <input value={newEvent.title} onChange={e => setNewEvent(p => ({...p, title: e.target.value}))} className="w-full px-4 py-2.5 border border-slate-200 rounded-xl focus:outline-none focus:ring-2 focus:ring-cyan-500" placeholder={t('events.titlePlaceholder')} />
              </div>
              <div className="grid grid-cols-2 gap-4">
                <div>
                  <label className="block text-xs font-bold text-slate-500 uppercase tracking-widest mb-1">{t('events.dateLabel')}</label>
                ...
                <div>
                  <label className="block text-xs font-bold text-slate-500 uppercase tracking-widest mb-1">{t('events.timeLabel')}</label>
              ...
              <div>
                  <label className="block text-xs font-bold text-slate-500 uppercase tracking-widest mb-1">{t('events.locationLabel')}</label>
                <input value={newEvent.location} onChange={e => setNewEvent(p => ({...p, location: e.target.value}))} className="w-full px-4 py-2.5 border border-slate-200 rounded-xl focus:outline-none focus:ring-2 focus:ring-cyan-500" placeholder={t('events.locationPlaceholder')} />
              </div>
              <div>
                <label className="block text-xs font-bold text-slate-500 uppercase tracking-widest mb-1">{t('events.typeLabel')}</label>
                <select value={newEvent.type} onChange={e => setNewEvent(p => ({...p, type: e.target.value}))} className="w-full px-4 py-2.5 border border-slate-200 rounded-xl focus:outline-none focus:ring-2 focus:ring-cyan-500">
                  {[t('events.typePersonal'), t('events.typeStudyGroup'), t('events.typeMeeting'), t('events.typeReminder'), t('events.typeOther')].map(t => <option key={t}>{t}</option>)}
                </select>
              </div>
              <div>
                <label className="block text-xs font-bold text-slate-500 uppercase tracking-widest mb-1">{t('events.descriptionLabel')}</label>
                <textarea value={newEvent.description} onChange={e => setNewEvent(p => ({...p, description: e.target.value}))} rows={3} className="w-full px-4 py-2.5 border border-slate-200 rounded-xl focus:outline-none focus:ring-2 focus:ring-cyan-500 resize-none" placeholder={t('events.descriptionPlaceholder')} />
              </div>
              <div className="flex gap-3 pt-2">
                <button type="button" onClick={() => { setShowAddModal(false); setAddError(''); }} className="flex-1 py-3 rounded-xl border border-slate-200 text-slate-600 font-bold text-sm hover:bg-transparent">{t('common:cancel')}</button>
                <button type="submit" className="flex-1 py-3 rounded-xl bg-slate-900 text-white font-bold text-sm hover:bg-slate-800">{t('events.addEventBtn')}</button>
              </div>
            </form>
          </div>
        </div>
