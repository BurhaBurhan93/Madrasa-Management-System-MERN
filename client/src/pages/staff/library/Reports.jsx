...
const StaffLibraryReports = () => {
  ...
  const borrowedRate =
    data.totalBooks > 0
      ? Math.round((data.borrowed / data.totalBooks) * 100)
      : 0;

  const returnedRate =
    data.totalBooks > 0
      ? Math.round((data.returned / data.totalBooks) * 100)
      : 0;
  ...
  return (
    <div className="w-full min-h-screen">
      ...
      {error && !loading && (
        <Card className="rounded-[28px] border border-rose-200 bg-rose-50 dark:bg-rose-900/20 dark:border-rose-800 mb-6">
          <div className="p-6">
