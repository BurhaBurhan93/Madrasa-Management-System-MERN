...
const StaffAnalyticsContent = ({ stats = [], charts = [], insight }) => (
  <div className="space-y-6">
    {stats.length > 0 && (
      <div className="grid grid-cols-1 gap-4 sm:grid-cols-2 xl:grid-cols-5">
        {stats.map((stat) => {
          ...
          return (
            <Card key={stat.label} className={`rounded-[28px] border bg-gradient-to-br shadow-none ${tone.card}`}>
              <div className="flex items-start justify-between gap-4">
                <div>
                  <p className="text-xs font-bold uppercase tracking-[0.2em] text-slate-500">{stat.label}</p>
                  <p className={`mt-3 text-3xl font-black tracking-tight ${tone.value}`}>{stat.value}</p>
                  {stat.helper ? <p className={`mt-2 text-sm font-medium ${tone.helper}`}>{stat.helper}</p> : null}
                </div>
                {Icon ? (
    ...
    {insight ? (
      <Card className="rounded-[28px] border border-slate-200 bg-gradient-to-r from-slate-50 via-white to-cyan-50/50 shadow-none">
        <div className="flex flex-col gap-3 lg:flex-row lg:items-center lg:justify-between">
          <div>
            <p className="text-xs font-bold uppercase tracking-[0.2em] text-cyan-700">{insight.eyebrow || 'Snapshot'}</p>
            <h3 className="mt-2 text-xl font-black tracking-tight text-slate-900">{insight.title}</h3>
            {insight.description ? <p className="mt-2 max-w-3xl text-sm leading-6 text-slate-600">{insight.description}</p> : null}
          </div>
          {insight.meta ? <div className="text-sm font-semibold text-slate-500">{insight.meta}</div> : null}
        </div>
      </Card>
    ) : null}
