...
const Reports = () => {
  ...
  const subtitle = useMemo(() => {
    if (filters.period === "daily") return "Daily kitchen summary and analytics";
    if (filters.period === "weekly") return "Weekly kitchen summary and analytics";
    return "Monthly kitchen summary and analytics";
  }, [filters.period]);

  return (
    <StaffPageLayout eyebrow="Kitchen" title="Kitchen Reports" subtitle={subtitle}>
      <div className="space-y-6">
        ...
        {loading ? (
          <div className="p-8 text-center text-sm text-slate-500 dark:text-slate-400">Loading report...</div>
        ) : error ? (
          ...
          <>
            <div className="grid grid-cols-2 gap-4 md:grid-cols-4">
              {[
                { label: "Total Purchases", value: `${report.totalPurchases.toLocaleString()} AFN`, icon: FiShoppingCart, color: "from-red-50 to-orange-50", iconBg: "bg-red-100 dark:bg-red-900/30", iconColor: "text-red-600" },
                { label: "Total Meals Served", value: report.totalMeals, icon: FiUsers, color: "from-blue-50 to-cyan-50", iconBg: "bg-blue-100 dark:bg-blue-900/30", iconColor: "text-blue-600" },
                { label: "Student Meals", value: report.totalStudentMeals, icon: FiUsers, color: "from-green-50 to-emerald-50", iconBg: "bg-green-100 dark:bg-green-900/30", iconColor: "text-green-600" },
                { label: "Staff Meals", value: report.totalStaffMeals, icon: FiUsers, color: "from-purple-50 to-fuchsia-50", iconBg: "bg-purple-100 dark:bg-purple-900/30", iconColor: "text-purple-600" },
              ].map((card) => {
            ...
            <div className="grid grid-cols-2 gap-4 md:grid-cols-4">
              {[
                { label: "Total Inventory Items", value: report.totalInventoryItems, icon: FiPackage, color: "from-slate-50 to-gray-50", iconBg: "bg-slate-100 dark:bg-slate-800", iconColor: "text-slate-700 dark:text-slate-200" },
                { label: "Low Stock Items", value: report.lowStockItems, icon: FiAlertTriangle, color: "from-yellow-50 to-amber-50", iconBg: "bg-yellow-100 dark:bg-yellow-900/30", iconColor: "text-yellow-600" },
                { label: "Waste Records", value: report.totalWasteRecords, icon: FiTrash2, color: "from-red-50 to-rose-50", iconBg: "bg-red-100 dark:bg-red-900/30", iconColor: "text-red-600" },
                { label: "Active Students", value: report.activeStudents, icon: FiUsers, color: "from-blue-50 to-indigo-50", iconBg: "bg-blue-100 dark:bg-blue-900/30", iconColor: "text-blue-600" },
              ].map((card) => {
                const Icon = card.icon;
                return (
                  <Card key={card.label} className={`rounded-2xl border border-slate-200 bg-gradient-to-br ${card.color} p-5 dark:border-slate-700 dark:bg-none dark:bg-slate-800/50`}>
            ...
            {report.budget && (
              <Card className="rounded-2xl border border-slate-200 p-6 dark:border-slate-700 dark:bg-slate-800/50">
                ...
                <div className="grid grid-cols-2 gap-4 md:grid-cols-4">
                  {[
                    { label: "Allocated", value: `${report.budget.allocatedAmount.toLocaleString()} AFN`, color: "text-blue-600 dark:text-blue-400", icon: FiDollarSign },
                    { label: "Approved", value: `${report.budget.approvedAmount.toLocaleString()} AFN`, color: "text-green-600 dark:text-green-400" },
                    { label: "Spent", value: `${report.budget.spentAmount.toLocaleString()} AFN`, color: "text-red-600 dark:text-red-400" },
                    { label: "Remaining", value: `${report.budget.remainingAmount.toLocaleString()} AFN`, color: "text-purple-600 dark:text-purple-400" },
                  ].map((card) => (
