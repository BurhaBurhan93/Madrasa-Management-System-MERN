...
export const getSuppliersConfig = (t) => ({
  title: t('staff.kitchen.suppliers.title', 'Suppliers'),
  subtitle: t('staff.kitchen.suppliers.subtitle', 'Manage supplier profiles with the same clean table and action system.'),
  endpoint: '/kitchen/suppliers',
  columns: [
    { key: 'name', header: t('staff.kitchen.suppliers.supplierName', 'Supplier Name') },
    { key: 'phone', header: t('common.phone', 'Phone') },
    { key: 'address', header: t('common.address', 'Address') },
    { key: 'itemsSupplied', header: t('staff.kitchen.suppliers.itemsSupplied', 'Items Supplied'), render: (value) => Array.isArray(value) && value.length ? value.join(', ') : '-' },
    { key: 'status', header: t('common.status', 'Status') }
  ],
  formFields: [
    { name: 'name', label: t('staff.kitchen.suppliers.supplierName', 'Supplier Name') },
    ...
    { name: 'itemsSupplied', label: t('staff.kitchen.suppliers.itemsSupplied', 'Items Supplied'), type: 'textarea', rows: 3 },
...
const Suppliers = () => {
  ...
  return <ListPage title={config.title} subtitle={config.subtitle} endpoint={config.endpoint} columns={config.columns} createPath="/staff/kitchen/suppliers/create" editPathForRow={(row) => `/staff/kitchen/suppliers/edit/${row._id}`} viewPathForRow={(row) => `/staff/kitchen/suppliers/view/${row._id}`} searchPlaceholder={t('staff.kitchen.suppliers.searchPlaceholder', 'Search suppliers...')} clientSidePagination={true} />;
};
