...
const StudentHostel = () => {
  ...
  const [applyData, setApplyData] = useState({ reason: '', roomPreference: '', moveInDate: '', notes: '' });
  const [applyStatus, setApplyStatus] = useState({ loading: false, success: false, error: '' });
  ...
  const fetchHostelData = async () => {
    try {
      const res = await apiFetch('/student/hostel');
      const data = await parseJsonSafe(res);
      if (data.success) setHostelData(data.data);
    } catch (error) {
  ...
  const fetchMealSchedule = async () => {
    try {
      const res = await apiFetch('/hostel/meals/upcoming');
      const data = await parseJsonSafe(res);
  ...
  const fetchExistingApplication = async () => {
    try {
      const res = await apiFetch('/student/hostel/application');
      const data = await parseJsonSafe(res);
      if (data.success && data.data) setExistingApplication(data.data);
    } catch {}
  };
  ...
  const handleApplySubmit = async (e) => {
    ...
    if (!applyData.reason.trim()) {
      setApplyStatus({ loading: false, success: false, error: 'Please provide a reason for your application.' });
      return;
    }
    ...
    try {
      ...
      setApplyStatus({ loading: false, success: false, error: err.message });
    }
  };
      ...
      {isApplyModalOpen && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/40 backdrop-blur-sm p-4">
          <div className="bg-white rounded-[32px] shadow-2xl w-full max-w-md p-8">
            ...
            {applyStatus.error && (
              <div className="mb-6 p-4 bg-rose-50 text-rose-800 rounded-2xl border border-rose-200">
                <p className="font-bold">{applyStatus.error}</p>
              </div>
            )}

            <form onSubmit={handleApplySubmit} className="space-y-4">
              <div>
                <label className="block text-xs font-bold text-slate-500 uppercase tracking-widest mb-1">Reason for Applying *</label>
                <textarea
                  required
              ...
              <div>
                <label className="block text-xs font-bold text-slate-500 uppercase tracking-widest mb-1">Room Preference</label>
                <input
                  value={applyData.roomPreference}
                  onChange={e => setApplyData(p => ({...p, roomPreference: e.target.value}))}
                  className="w-full px-4 py-2.5 border border-slate-200 rounded-xl focus:outline-none focus:ring-2 focus:ring-cyan-500"
                  placeholder="e.g., Single Room, Floor 2, etc."
                />
              </div>
              <div>
                <label className="block text-xs font-bold text-slate-500 uppercase tracking-widest mb-1">Preferred Move-in Date</label>
                <input
                  type="date"
                  value={applyData.moveInDate}
                  onChange={e => setApplyData(p => ({...p, moveInDate: e.target.value}))}
                  className="w-full px-4 py-2.5 border border-slate-200 rounded-xl focus:outline-none focus:ring-2 focus:ring-cyan-500"
                />
              </div>
              <div>
                <label className="block text-xs font-bold text-slate-500 uppercase tracking-widest mb-1">Additional Notes</label>
                <textarea
  ...
  // Render the appropriate main content + modals
  return (
    <>
      {!isResident ? (
      ...
      {/* Always include the modals at the end */}
      {renderModals()}
    </>
  );
};
